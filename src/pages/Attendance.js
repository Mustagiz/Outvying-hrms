        ...
        for (const date of dates) {
          ...
          if (clockIn) {
            ...
            attendanceUpdate = {
              ...
              updatedAt: serverTimestamp() // We need serverTimestamp imported or passed
            };
          } else {
            // If only updating status/remarks without time, mostly used for 'Present' marking
            // But usually Manual Attendance implies time. If no time, maybe just mark Present?
            // For now, let's assume if no time, we just set status if we had one, but the logic above depends on Time.
            // If no time is provided, we skip or set default?
            // The previous logic required clockIn. Let's keep it required implicitly or handle empty.
            if (!clockIn && !clockOut) continue;
          }

          // Update Firestore
          // Check for existing record
          // We need query/getDocs/updateDoc/setDoc from firebase/firestore
          // We can import them dynamically inside if not available in scope
          const { collection, query, where, getDocs, doc, setDoc, updateDoc, serverTimestamp } = await import('firebase/firestore');
          const { db } = await import('../config/firebase'); // Re-import to be safe inside async
          ...
          if (!snapshot.empty) {
            ...
            await updateDoc(docRef, attendanceUpdate);
          } else {
            const newId = `${empId}-${date}`; // Standard ID format
